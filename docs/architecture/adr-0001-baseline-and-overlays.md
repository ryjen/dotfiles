...
2. Ignored `home/USERNAME/user.nix` selects user-wide capabilities and local identity.
...
- `home/USERNAME/user.example.nix` is tracked and kept current as the template for local `user.nix`.
- `home/USERNAME/profiles/` contains host-role deltas, capability constraints, paths, and machine-specific values.
- personal Git identity is exposed through typed module options and configured in `user.nix`.
...
- user selections and local identity have one documented override surface
...
- documentation can describe one stable architecture instead of migration exceptions

Tradeoffs:

- one more layer of indirection exists between module definitions and concrete host behavior
