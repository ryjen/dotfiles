...
in
{
  options.dotfiles.headroom = {
    ...
    mcp = {
      enable = lib.mkEnableOption "Headroom MCP server tool registration";

      package = lib.mkOption {
        type = lib.types.str;
        default = "${config.home.homeDirectory}/.local/libexec/headroom-mcp";
        description = ''
          Path to the Headroom MCP launcher. Runs `headroom mcp serve`, which
          exposes the headroom_compress, headroom_retrieve, and headroom_stats
          tools to MCP clients.

          Requires Headroom installed with the `mcp` extra
          (`pip install "headroom-ai[mcp]"` or the equivalent uv tool install).
        '';
      };
    };
  };
  ...
  config = lib.mkMerge [
    ...
    # MCP is a stdio server launched on demand by each client, not a background
    # daemon, so it installs the launcher without a systemd user service.
    (lib.mkIf cfg.mcp.enable {
      home.file.".local/libexec/headroom-mcp" = {
        source = ../../files/home/.local/libexec/headroom-mcp;
        executable = true;
      };
    })
